...
const ProductClient: React.FC<ProductClientProps> = ({ product }) => {
  ...
  const imageUrl = product.featureImage
    ? urlFor(product.featureImage).url()
    : "";
  const additionalImageUrls = product.additionalImages
    ? product.additionalImages.map((img: any) => urlFor(img).url())
    : [];

  const displayPrice = product.salePrice
    ? `Sale: £${product.salePrice} (was £${product.price})`
    : `Price: £${product.price}`;

  const handleBuyNow = () => {
    addToCart(); // Ensure the product is added to the cart
              ...
              <div className="mt-6">
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 px-6 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-500 transition duration-300"
                  >
                    Buy Now
                  </button>
                </div>
